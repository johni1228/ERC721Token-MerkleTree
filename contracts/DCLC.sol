...
interface IERC2981Royalties {
    function royaltyInfo(uint256 _tokenId) external view returns (address _receiver, uint256 _royaltyAmount);
}
...
abstract contract ERC2981Royalties is ERC2981Base {
    ...
    function _setRoyalties(uint256 tokenId, address recipient, uint256 value) internal {
        ...
        RoyaltyInfo memory _royalties = RoyaltyInfo(recipient, uint24(value));
        royalties[tokenId] = _royalties;

    }
	
    function royaltyInfo(uint256 tokenId) external view override returns (address receiver, uint256 royaltyAmount){
        RoyaltyInfo memory _royalties = royalties[tokenId];
        receiver = _royalties.recipient;
        royaltyAmount = _royalties.amount;
    }
}
...
contract DCLC is ERC721Enum, Ownable, ReentrancyGuard{
  ...
  constructor() ERC721P("Digital Celebrities Longevity Coin", "DCLC") {}
  ...
  function updateTotalSupply(uint256 newSupply) external onlyOwner {
	...
	function setRoyalties(uint256 _tokenId, address recipient, uint256 value) public onlyOwner{
        _setRoyalties(_tokenId, recipient, value);
    }
}